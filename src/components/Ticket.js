...
import checkForProfanity from "../profanityDetector";
...
class Ticket extends React.Component {
  ...
  // Combine all text content for profanity checking
  const textToCheck = `${ticketName} ${ticketSubject} ${ticketDetails}`;
  ...
  try {
    ...
    const newTicket = {
      ...
      items: { j: "" },
      comments: { p: "" },
      // Set flag based on profanity check result
      flag: profanityResult.containsProfanity ? "true" : "false",
      createdBy: {
    ...
    ticketsRef
      ...
      .then(() => {
        ...
        this.closeModalT();
        console.log("Ticket added", profanityResult.containsProfanity ? "(flagged for abusive content)" : "");
      })
      .catch((err) => {
    ...
    // Continue with submission even if profanity check fails
    ...
    const newTicket = {
      ...
      items: { j: "" },
      comments: { p: "" },
      flag: "false", // Default to not flagged if check fails
      createdBy: {
        name: user.displayName,
        email: user.email,
        faculty: this.state.faculty,
    ...
    ticketsRef
      ...
      .then(() => {
        ...
        this.closeModalT();
        console.log("Ticket added (profanity check failed)");
      })
      .catch((err) => {
        console.error("Error adding ticket:", err);
        this.setState({ loading: false });
